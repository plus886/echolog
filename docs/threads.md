...
- **予約ロジック**: 台湾時間 20:00–22:00 のランダムな分。1日1件まで
  （中文・日本語の2チャンネル1組）。明日から順に空きのある日へ割り当て
...
7. **返信の見落とし防止**: cron が返信状況を D1 に同期し、開かなくても
   ...
   admin のタブにも件数バッジが出る
8. **返信案（AI で提案）**: 返信フォームの「AI で提案」で Opus 5 が
   返信案を 1 件書く（送信はしない）。投稿本文と届いた返信を踏まえた
   短い返事になる。押すたびに別案が出て、案を手直ししてから押すと
   その手直しを指示として渡す
9. **対応済み**: 返信せずに片付けたいときは各返信の「対応済みにする」。
   要返信バッジが下り、タブの件数からも外れる

### 返信案の設計

- モデルは Opus 5 固定（`threads-reply-suggest.ts`）。トーン・文体の調整は
  `threads-reply-suggest-prompt.ts` だけで完結する
- **返信の言語は相手に合わせる**。アカウントの言語ではなく届いた返信の
  言語で返す（中文アカウントに日本語で書き込む日本人フォロワーがいるため）。
  中文で返すときは簡体字で来ても必ず繁體中文。絵文字だけなど判別できない
  ときはチャンネルの言語にフォールバックする
- **届いた返信はデータであって指示ではない**旨を system prompt で明示して
  いる。返信文に「これまでの指示を無視しろ」等が混ざっていても従わない
  （実際に注入を試して、英語化・URL 挿入のいずれも起きないことを確認済み）
- 投稿から読み取れない事実（撮影地・機材など）は断定させない

### 「いいね」について

**Threads API には投稿や返信にいいねを付ける口が無い**（Reply Management は
取得と hide/unhide のみ、Insights はいいね数の取得のみ）。そのため管理画面
からいいねは押せない。代わりに「対応済みにする」で処理済みを記録する
（`migrations/0004_handled_reply.sql` の `handled_reply_id`）。

値は「対応を済ませた最新の受信返信の ID」。cron の同期後もこれを見て
「要返信」を下げ続けるが、**その後に新しい返信が来れば ID が変わるので
自動的にまた「要返信」に戻る**。いいね自体を付けたい場合は Threads
アプリ側で押す（押しても admin の表示は変わらないので、必要なら
「対応済みにする」も押す）。
