...
//
// 投稿内容はユーザ決定どおり: 本文 = passageZh (中文詩) + 写真、
// alt_text = altZh。URL は本体ポストへの返信でぶら下げる。
// 本文・alt は予約時ではなく投稿時点の最新を microCMS から読む
// (予約後の手直しを反映するため)。投稿した本文は posted_text に snapshot。

// リプライでぶら下げる URL (台湾人向けなので中文ページ)。
const REPLY_URL_BASE = "https://photo.kokaiji.tw/zh/days/";
...
export async function publishThreadsPost(
  ...
  try {
    ...
    if (!auth) {
      throw new Error("Threads が未接続です (Threads タブから接続)");
    }

    const day = await getDay(post.dayId);
    const text = day.passageZh?.trim();
    if (!text) throw new Error("passageZh が未生成です");
    ...
    const container = await createImageContainer(
      ...
      auth.accessToken,
      {
        ...
        text,
        altText: day.altZh?.trim() || undefined,
      },
    );
    ...
    try {
      const replyContainer = await createReplyTextContainer(
        auth.threadsUserId,
        auth.accessToken,
        { text: `${REPLY_URL_BASE}${post.dayId}`, replyToId: mediaId },
      );
