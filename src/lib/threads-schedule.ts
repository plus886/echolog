...
// 20:00–22:00 のどこかにランダムで置く。1 日 1 件が上限で、埋まっていれば
// 翌日以降へ繰り越す。副作用のない純関数なのでテストしやすく、cron でも
// action でも同じ結果になる。

const TAIPEI_OFFSET_MIN = 8 * 60;
const WINDOW_START_HOUR = 20; // 台湾時間 20:00
const WINDOW_MINUTES = 120; // 20:00–22:00 (両端含む)
const MIN_GAP_MS = 60 * 60 * 1000; // 予約どうしは 60 分以上あける
// 1 日 1 件 (= 2 チャンネル 1 組)。手動で日時を動かした予約もこの上限の
// 判定に参加するので、詰め込みたい場合はダッシュボードで直接ずらす。
